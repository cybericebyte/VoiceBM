...
**Active 2.0 contains three breaking changes.** Read the Breaking Changes section
...
- Default is **0 (no rollover)** — existing behavior is unchanged until a value
  is set. The cap is read at enrollment time, so changes take effect immediately
  without a restart.
...
| F-02 | Feature | Gallery rollover cap (`voicebm/gallery_max`) | Added (default off) |

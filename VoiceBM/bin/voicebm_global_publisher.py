...
from voicebm_config import get_mqtt_config, get_active_lead_trim_ms, update_voicebm_config_key, get_inject_identity, get_transcript_preferred
...
TRANSCRIPT_PREFERRED_TOPIC = "voicebm/transcript_preferred"
...
# Global state
# config.json is the single source of truth for both switches. Seed the live
# globals from it at import so the dashboard and HA stay in sync across restarts.
inject_identity_enabled = get_inject_identity()
transcript_preferred_enabled = get_transcript_preferred()
...
def _write_person_override_file(person_id, threshold):
    """Write/update one person's override in the PERSON_OVERRIDES map of
    thresholds.json. Atomic (temp + os.replace), preserving every other key
    (MATCH_T_ACTIVE, GALLERY_MAX, other people's overrides)."""
    try:
        if os.path.exists(THRESHOLD_FILE):
            with open(THRESHOLD_FILE, "r") as f:
                data = json.load(f)
...
def resolve_person_threshold(person_id):
    """Resolve a person's custom threshold from BOTH sources:
      - the in-memory person_thresholds dict (live MQTT/HA path), and
      - the PERSON_OVERRIDES map in thresholds.json (dashboard file path).
    The in-memory value wins if both are set (a live HA change beats the file).
    Returns the float override or None if the person has no custom threshold.
    """
...
def publish_discovery(client):
    ...
    # ID Injection Switch — config.json is the source of truth.
    # Prepends the identity tag to the transcript when ON.
    inject_switch_config = {
        "name": "ID Injection",
    ...
    client.publish(
        ...
        json.dumps(inject_switch_config),
        qos=1,
        retain=True,
    )
    client.publish(INJECT_STATE_TOPIC, "ON" if get_inject_identity() else "OFF", qos=1, retain=True)
    print(f"  Initialized ID injection: {'ON' if get_inject_identity() else 'OFF'}")

    # Transcript Preferred Switch — config.json is the source of truth.
    # Gates the voicebm/transcript/preferred publish (debug topic is never gated).
    # Turn OFF to keep bridge-driven speech out of the preferred topic.
    transcript_preferred_config = {
        "name": "Transcript Preferred",
        "unique_id": "voicebm_transcript_preferred",
        "command_topic": f"{TRANSCRIPT_PREFERRED_TOPIC}/set",
        "state_topic": TRANSCRIPT_PREFERRED_TOPIC,
        "payload_on": "ON",
        "payload_off": "OFF",
        "icon": "mdi:transcribe",
        "device": device,
    ...
    client.publish(
        f"{discovery_prefix}/switch/voicebm_transcript_preferred/config",
        json.dumps(transcript_preferred_config),
        qos=1,
        retain=True,
    )
    client.publish(TRANSCRIPT_PREFERRED_TOPIC, "ON" if get_transcript_preferred() else "OFF", qos=1, retain=True)
    print(f"  Initialized transcript preferred: {'ON' if get_transcript_preferred() else 'OFF'}")
...
def handle_person_threshold_set(client, userdata, msg):
    ...
    try:
        ...
        # Also persist to the PERSON_OVERRIDES map in thresholds.json so the
        # override survives a restart and is visible to the dashboard (which is
        # file-based and may have no broker). Two writers, one file.
...
def handle_inject_set(client, userdata, msg):
    """Handle ID Injection switch; persist to config.json (single source of truth)."""
    global inject_identity_enabled
    try:
        command = msg.payload.decode("utf-8")
        new_enabled = command == "ON"
        print(f"ID Injection changed to: {command}")
        if update_voicebm_config_key("inject_identity", new_enabled):
            inject_identity_enabled = new_enabled
            print(f"  Updated config.json: voicebm.inject_identity = {new_enabled}")
            client.publish(INJECT_STATE_TOPIC, "ON" if new_enabled else "OFF", qos=1, retain=True)
    except Exception as e:
        print(f"Error handling injection change: {e}")


def handle_transcript_preferred_set(client, userdata, msg):
    """Handle Transcript Preferred switch; persist to config.json (single source of truth)."""
    global transcript_preferred_enabled
    try:
        command = msg.payload.decode("utf-8")
        new_enabled = command == "ON"
        print(f"Transcript Preferred changed to: {command}")
        if update_voicebm_config_key("transcript_preferred", new_enabled):
            transcript_preferred_enabled = new_enabled
            print(f"  Updated config.json: voicebm.transcript_preferred = {new_enabled}")
            client.publish(TRANSCRIPT_PREFERRED_TOPIC, "ON" if new_enabled else "OFF", qos=1, retain=True)
    except Exception as e:
        print(f"Error handling transcript preferred change: {e}")
...
def on_connect(client, userdata, flags, reason_code, properties):
    ...
    if reason_code == 0:
        ...
        client.subscribe(f"{TRANSCRIPT_PREFERRED_TOPIC}/set", qos=1)
...
def on_message(client, userdata, msg):
    ...
    elif topic == f"{INJECT_STATE_TOPIC}/set":
        handle_inject_set(client, userdata, msg)
    elif topic == f"{TRANSCRIPT_PREFERRED_TOPIC}/set":
        handle_transcript_preferred_set(client, userdata, msg)
